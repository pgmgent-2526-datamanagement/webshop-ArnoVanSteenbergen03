<x-layout>
	<x-slot:seo>
		<x-seo title="LEGO Resale Shop - Browse All Products"
		...
		<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
			@forelse($products as $product)
				<div
					...
					<div class="relative h-72 bg-gray-50 flex-shrink-0">
						@if($product->image)
							<img src="{{ asset($product->image) }}" alt="{{ $product->name }}"
								class="w-full h-full object-contain p-6" loading="lazy"
								style="image-rendering: -webkit-optimize-contrast; image-rendering: crisp-edges;"
								onerror="this.onerror=null; this.src='https://via.placeholder.com/600x600/0066CC/FFFFFF?text={{ urlencode($product->name) }}';">
						@else
					...
					<div class="p-4 flex flex-col flex-grow">
						...
						<p class="text-sm text-gray-600 mb-3 line-clamp-2 flex-grow">{{ $product->description }}
						</p>
						...
						@if($product->tags->count() > 0)
							<div class="flex flex-wrap gap-1 mb-3">
								@foreach($product->tags->take(3) as $tag)
									<span class="text-xs bg-gray-200 text-gray-700 px-2 py-1 rounded">{{ $tag->name }}
									</span>
								@endforeach
							</div>
						@endif
							...
							<div class="flex gap-2">
								@if($product->stock > 0)
									<form action="{{ route('cart.add', $product->id) }}" method="POST">
										@csrf
										<button type="submit"
											class="bg-green-500 hover:bg-green-600 text-white px-3 py-2 rounded text-sm font-semibold transition-colors flex items-center gap-1">
