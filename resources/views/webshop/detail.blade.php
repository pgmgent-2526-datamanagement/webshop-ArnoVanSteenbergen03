<x-layout>
	<x-slot:seo>
		<x-seo :title="$product->name" :description="$product->description . ' - ' . $product->category->name . ' LEGO set with ' . ($product->piece_count ? number_format($product->piece_count) . ' pieces' : 'quality pieces') . '. Condition: ' . ucfirst($product->condition) . '. Price: €' . number_format($product->price, 2)"
			:image="$product->image ? asset($product->image) : null" type="product" :schema="[
	...
	<div class="container mx-auto px-4 py-8 max-w-7xl">
		<a href="{{ route('webshop.list') }}"
			class="inline-flex items-center text-blue-300 hover:text-blue-100 mb-6 transition-colors">
		...
		<div class="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
			...
			<div class="flex flex-col">
				<p class="text-sm text-blue-300 uppercase tracking-wide mb-2">{{ $product->category->name }}
				</p>

				<h1 class="text-4xl font-bold text-white mb-4">{{ $product->name }}
				</h1>
				...
				<div class="mb-6">
					...
					<p class="text-blue-100 leading-relaxed">{{ $product->description }}
					</p>
				</div>
				...
				<div class="mt-8">
					@if($product->stock > 0)
						<form action="{{ route('cart.add', $product->id) }}" method="POST">
							@csrf
							<button type="submit"
								class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 px-8 rounded-lg transition-colors duration-200 transform hover:scale-105">
								Add to Cart
							</button>
						</form>
					@else
						<button class="w-full bg-gray-300 text-gray-500 font-bold py-4 px-8 rounded-lg cursor-not-allowed"
							disabled>
							Out of Stock
						</button>
					@endif
